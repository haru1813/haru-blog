...
비슷한 도구로 **ModelMapper**도 있다. 근본 차이는 **"언제 일하느냐"**다. 이 하나에서 나머지가 다 갈린다.

```
MapStruct   → 컴파일할 때 "코드를 생성"     (미리 만들어둠)
ModelMapper → 실행할 때 "리플렉션으로 분석"  (그때그때 알아냄)
```

**ModelMapper**는 런타임 리플렉션으로 동작한다. 인터페이스 선언조차 없이 바로 쓴다.

```java
ModelMapper mm = new ModelMapper();
UserResponse dto = mm.map(user, UserResponse.class);
// 실행 시점에 두 클래스의 필드를 "들여다보고" 이름 맞는 것끼리 자동 복사
```

> **리플렉션(reflection)** = 실행 중에 클래스 구조(필드·메서드)를 들여다보고 조작하는 기능. 유연하지만 매번 분석하니 느리고, 컴파일 체크가 안 된다.

시작은 ModelMapper가 편하다(설정 0). 하지만 그 편함엔 대가가 있다.

| | **MapStruct** | ModelMapper |
|---|---|---|
| 동작 시점 | **컴파일 타임** | 런타임 |
| 방식 | **코드 생성** | 리플렉션 |
| 속도 | 빠름 (그냥 메서드 호출) | 느림 (매번 리플렉션 분석) |
| 오류 발견 | **컴파일 때** (빌드 실패) | 런타임 (실행 중 터짐) |
| 투명성 | 생성 코드를 **눈으로** 봄 | 블랙박스 (안 보임) |
| 시작 편의 | 인터페이스 선언 필요 | 설정 없이 바로 |

실무가 MapStruct로 기우는 건 두 가지 때문이다.

- **성능** — DTO 변환은 API 요청마다 **자주** 일어난다. 리플렉션(ModelMapper) 비용이 쌓이면 느려지지만, MapStruct는 `new + getter`라 부담이 없다
- **안전성** — ModelMapper는 필드를 잘못 맞춰도 **컴파일은 통과**하고 운영 중에 조용히 틀린 값이 들어가거나 터진다. MapStruct는 **빌드할 때** 걸려서 배포 전에 잡힌다

한마디로 — **ModelMapper는 "편하지만 런타임에 위험", MapStruct는 "조금 번거롭지만 컴파일 때 안전하고 빠르다"**. 그래서 진지한 프로젝트일수록 MapStruct로 기운다. 생성된 코드가 그냥 평범한 자바라, 디버깅도 쉽고.
