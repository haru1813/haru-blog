...
## 왜 생성자 주입이 정석인가 — 필드 주입과 비교

앞에서 "생성자 주입이 권장"이라고 했는데, 그 이유가 다섯 가지나 된다. 그리고 이건 `final`과 직결된다.

`final` 필드는 한 번 값이 들어가면 못 바꾸는(불변) 필드라 **생성자에서만** 초기화할 수 있다. 즉 **`final`을 쓸 수 있다는 것 자체가 생성자 주입의 장점**이다. 필드 주입(`@Autowired private ...`)으로는 `final`을 못 쓴다.

1. **불변성(final)** — 주입 후 의존성이 바뀌지 않는다. 실수로 재할당하는 사고를 컴파일러가 막는다.
2. **필수 의존성 보장** — 생성자에서 받으니 객체가 만들어지는 순간 모든 의존이 확정된다. `null`인 반쪽짜리 객체가 존재할 수 없다.
3. **순환 참조 조기 발견** — A와 B가 서로를 필요로 하는 순환 의존이 있으면, 생성자 주입은 앱이 뜨는 순간 에러로 알려준다. 필드 주입은 런타임에 그 지점을 호출할 때야 늦게 터진다.
4. **테스트 용이** — `new`로 가짜 의존을 넣어 스프링 없이 단위 테스트할 수 있다.

```java
OrderService service = new OrderService(new FakeRepository()); // 스프링 없이 주입
```

5. **의존성 명시** — 생성자 시그니처에 필요한 의존이 다 드러난다. 파라미터가 지나치게 많아지면 *"이 클래스가 책임이 과하다"*는 리팩터링 신호도 준다.

| | 생성자 주입 | 필드 주입(`@Autowired`) |
|---|---|---|
| 불변성(final) | ✅ 가능 | ❌ 불가 |
| 필수 의존성 보장 | ✅ | ❌ (null 순간 존재) |
| 순환 참조 | ✅ 시작 시 발견 | ❌ 런타임에 늦게 |
| 테스트 | ✅ `new`로 간단 | ❌ 스프링·리플렉션 필요 |
| 의존성 명시 | ✅ 생성자에 드러남 | ❌ 필드에 숨음 |

생성자 주입의 **유일한 단점**은 의존이 많아지면 생성자 코드가 길어진다는 것 — 그런데 이건 롬복이 없애 준다.
...
- **생성자 주입이 정석인 이유**: 불변(final) + 필수 의존성 보장 + 순환참조 조기발견 + 테스트 용이 + 의존성 명시
- **실무 표준**: 생성자 주입 + 롬복 `@RequiredArgsConstructor` (생성자 주입의 유일한 단점인 코드량을 자동화로 해결)
